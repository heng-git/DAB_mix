...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
        ...
        if(is_main_process()):
            backbone.load_state_dict(torch.load('resnet50-0676ba61.pth'),strict=False)
...
class Joiner(nn.Sequential):
    ...
    def forward(self, tensor_list: NestedTensor):
        xs = self[0](tensor_list)#即经过backbone后的输出结构  self[0]即backbone
        out: List[NestedTensor] = []
        pos = []
        for name, x in xs.items():
            out.append(x)#分别保存经过0,1,2,3，。。。层之后的结果
            # position encoding
            pos.append(self[1](x).to(x.tensors.dtype))#同上
