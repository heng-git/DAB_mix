...
class DABDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        self.refpoint_embed = nn.Embedding(num_queries, query_dim)#生成随机向量100个4维向量
        ...
        self.input_proj = nn.Conv2d(backbone.num_channels, hidden_dim, kernel_size=1)#num_channels = 512 if name in ('resnet18', 'resnet34') else 2048
        ...
        for box_embed in self.bbox_embed:
            nn.init.constant_(box_embed.layers[-1].bias.data, 0.0)
        #########################################
    def forward(self, samples: NestedTensor):
        ...
        if isinstance(samples, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)#为了能够统一batch中所有图像的尺寸，以便形成一个batch，我们需要的到其中的最大尺度(在所有维度上）
                #然后对尺度较小的图像进行padding填充，同时设置mask以指示哪些部分是padding来的，以便后续模型能够在有效区域学习目标，相当于加了一部分先验知识。
...
##############################
        outputs_class = self.class_embed[0](hs)
        ###########################

        a,b=outputs_class[-1],outputs_coord[-1]
